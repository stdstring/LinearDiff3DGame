...
namespace LinearDiff3DGame.Geometry3D.PolyhedronGraph
{
    ...
    public class Polyhedron3DGraph_ScaleTransformer_Test
    {
        ...
        {
            IPolyhedron3D cube = CreatePolyhedron(new Point3D(1, 1, 1),
                                                 new Point3D(1, -1, 1),
                                                 new Point3D(-1, 1, 1),
                                                 new Point3D(-1, -1, 1),
                                                 new Point3D(1, 1, -1),
        ...
        {
            IPolyhedron3D pyramid = CreatePolyhedron(new Point3D(1, 1, 1),
                                                    new Point3D(1, 1, 0),
                                                    new Point3D(-1, 1, 0),
                                                    new Point3D(1, -1, 0),
                                                    new Point3D(-1, -1, 0));
        ...
        private IPolyhedron3D CreatePolyhedron(params Point3D[] vertexes)
        {
            return new Polyhedron3DFromPointsFactory(approxComp).CreatePolyhedron(vertexes);
        }

        private void TestScaleTransformer(IPolyhedron3D sourcePolyhedron, Vector3D direction, Double scalingRatio)
        {
            ...
            // polyhedron 2 graph :
            IPolyhedron3DGraph graph = new Polyhedron3DGraphFactory().CreatePolyhedronGraph(sourcePolyhedron);
            ...
            // graph 2 polyhedron
            IPolyhedron3D intermediatePolyhedron = new Polyhedron3DFromGraphFactory(approxComp, new LESKramer3Solver()).CreatePolyhedron(graph);
            // reverse transform polyhedron
            IPolyhedron3D destPolyhedron = TransformPolyhedron(intermediatePolyhedron, reverseTransform);
