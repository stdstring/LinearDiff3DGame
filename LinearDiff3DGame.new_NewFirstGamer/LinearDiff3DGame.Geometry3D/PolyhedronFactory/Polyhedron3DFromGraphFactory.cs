...
namespace LinearDiff3DGame.Geometry3D.PolyhedronFactory
{
    public class Polyhedron3DFromGraphFactory
    {
        ...
        {
            IDictionary<PointIndexTriplet, PolyhedronVertex3D> calculatedVertexes = new Dictionary<PointIndexTriplet, PolyhedronVertex3D>();
            ...
            graph.NodeList.Aggregate(planes, (accum, node) =>
                {
                    ...
                    Plane3D plane = new Plane3D(planeNormal.XCoord, planeNormal.YCoord, planeNormal.ZCoord, -supportFuncValue);
            ...
            foreach(Polyhedron3DGraphNode node in graph.NodeList)
            {
                ...
                {
                    Polyhedron3DGraphNode conn = node.ConnectionList[connIndex];
                    Polyhedron3DGraphNode connNext = node.ConnectionList.GetNextItem(connIndex);
                    PointIndexTriplet triplet = PointIndexTriplet.GetProperPointIndexTriplet(node.ID, conn.ID, connNext.ID);
                    PolyhedronVertex3D vertex;
                ...
                PolyhedronSide3D side = new PolyhedronSide3D(sideVertexList, sideID, node.NodeNormal);
                sideList.Add(side);
            }
        ...
        public Polyhedron3D CreatePolyhedron2(Polyhedron3DGraph graph)
        {
        ...
        private PolyhedronVertex3D FindVertex4Point(IList<PolyhedronVertex3D> vertexList, Point3D point)
        {
            ...
            for(Int32 vertexIndex = 0; vertexIndex < vertexList.Count; ++vertexIndex)
            {
                ...
                if(approxComparer.EQ(point.XCoord, currentVertex.XCoord) &&
                   approxComparer.EQ(point.YCoord, currentVertex.YCoord) &&
                   approxComparer.EQ(point.ZCoord, currentVertex.ZCoord))
                {
        ...
        public static PolyhedronVertex3D FindVertex(ICollection<PolyhedronVertex3D> vertexes,
        ...
        private static Double VertexApproxEqual(PolyhedronVertex3D vertex1, PolyhedronVertex3D vertex2, ApproxComp approxComp)
        {
        ...
        private static Boolean VertexApproxEqual(PolyhedronVertex3D vertex, Point3D point, ApproxComp approxComp)
        {
            Double squaredDistance = (vertex.XCoord - point.XCoord) * (vertex.XCoord - point.XCoord) +
                                     (vertex.YCoord - point.YCoord) * (vertex.YCoord - point.YCoord) +
                                     (vertex.ZCoord - point.ZCoord) * (vertex.ZCoord - point.ZCoord);
