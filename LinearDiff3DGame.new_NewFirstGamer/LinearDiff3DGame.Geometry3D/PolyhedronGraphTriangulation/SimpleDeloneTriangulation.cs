...
namespace LinearDiff3DGame.Geometry3D.PolyhedronGraphTriangulation
{
    ...
    {
        // TODO : реFUCKторинг
        public void Action(IList<Polyhedron3DGraphNode> sector,
                           IList<Pair<Polyhedron3DGraphNode, Polyhedron3DGraphNode>> connSet)
        {
            ...
            for(Int32 node1Index = 0; node1Index < sector.Count; ++node1Index)
            {
                ...
                for(Int32 node2Index = node1Index + 2; node2Index < sector.Count; ++node2Index)
                {
                    ...
                    Double cos12a = Vector3DUtils.CosAngleBetweenVectors(vector2a,
                                                                         vector21);
                    Double maxCosValue = GetMaxValue(Math.Abs(cos21b),
                                                     Math.Abs(cos21a),
                                                     Math.Abs(cos12b),
                                                     Math.Abs(cos12a));
            ...
            IList<Polyhedron3DGraphNode> subSector = new List<Polyhedron3DGraphNode>();
        ...
        private static Boolean CheckNodeVisibility(IList<Polyhedron3DGraphNode> sectorNodes,
                                                   Polyhedron3DGraphNode node1,
                                                   Polyhedron3DGraphNode node2)
        {
            Vector3D normal12 = Vector3D.VectorProduct(node1.NodeNormal, node2.NodeNormal);

            for(Polyhedron3DGraphNode node = node1;
                node != node2;
                node = sectorNodes.GetNextItem(node))
            {
                if(ReferenceEquals(node, node1))
                    continue;
                Double scalarProductValue = Vector3D.ScalarProduct(node.NodeNormal, normal12);
                if(scalarProductValue >= 0)
                    return false;
            }

            for(Polyhedron3DGraphNode node = node2;
                node != node1;
                node = sectorNodes.GetNextItem(node))
            {
        ...
        private static Double GetMaxValue(params Double[] doubleValues)
        {
        ...
        private static Double GetMinValue(params Double[] doubleValues)
        {
