...
{
    public class BridgeSerializer : ISerializer<Pair<Int32, IEnumerable<Pair<Double, Polyhedron3D>>>>
    {
        public void Serialize(Stream storage, Pair<Int32, IEnumerable<Pair<Double, Polyhedron3D>>> bridge)
        {
            EnumerableBinarySerializer<Pair<Double, Polyhedron3D>> sectionsSerializer = GetSerializer();
            Byte[] header = Encoding.ASCII.GetBytes(headerText);
            storage.Write(header, 0, header.Length);
            Pair<Int32, IEnumerable<Pair<Double, Polyhedron3D>>> data =
                new Pair<Int32, IEnumerable<Pair<Double, Polyhedron3D>>>(bridge.Item1, bridge.Item2);
            sectionsSerializer.Serialize(storage, data);
        }

        public Pair<Int32, IEnumerable<Pair<Double, Polyhedron3D>>> Deserialize(Stream storage)
        {
            EnumerableBinarySerializer<Pair<Double, Polyhedron3D>> sectionsSerializer = GetSerializer();
        ...
        protected virtual EnumerableBinarySerializer<Pair<Double, Polyhedron3D>> GetSerializer()
        {
            ...
            return new EnumerableBinarySerializer<Pair<Double, Polyhedron3D>>(pairSerializer);
        }
