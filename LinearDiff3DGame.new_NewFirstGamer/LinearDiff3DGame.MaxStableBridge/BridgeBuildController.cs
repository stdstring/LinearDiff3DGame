...
namespace LinearDiff3DGame.MaxStableBridge
{
    public class BridgeBuildController
    {
        ...
        public IList<Pair<Double, Polyhedron3D>> GenerateBridge(Double finishTime)
        {
            IList<Pair<Double, Polyhedron3D>> bridge = new List<Pair<Double, Polyhedron3D>>();
            while(approxComp.LT(builder.InverseTime, finishTime))
            {
                bridge.Add(new Pair<Double, Polyhedron3D>(builder.InverseTime, builder.CurrentTSection));
                builder.NextIteration();
            }
            return bridge;
        }

        public IEnumerable<Pair<Double, Polyhedron3D>> CalculateBridge(Double finishTime,
                                                                       Boolean includeBoundary)
        {
            yield return new Pair<Double, Polyhedron3D>(builder.InverseTime, builder.CurrentTSection);
            Func<Double, Boolean> breakCalculation = time => includeBoundary ? approxComp.GT(time, finishTime) : approxComp.GE(time, finishTime);
