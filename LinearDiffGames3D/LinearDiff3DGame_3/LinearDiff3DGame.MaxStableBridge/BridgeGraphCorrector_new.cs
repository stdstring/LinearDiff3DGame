        ...
        public Polyhedron3DGraph CheckAndCorrectBridgeGraph(SuspiciousConnectionSet connSet, Polyhedron3DGraph graph)
        {
            ...
            while (connSet.Count > 0)
            {
                ...
                else
                {
                    ...
                    Matrix lambda123 = CalcLambda123(node1.NodeNormal, node2.NodeNormal, node3.NodeNormal, node4.NodeNormal, out matrixError);
...
#warning может более специализированное исключение
        ...
        /// <returns></returns>
        private Boolean CheckConnConvexity(Polyhedron3DGraphNode node1, Polyhedron3DGraphNode node2, Polyhedron3DGraphNode node3, Polyhedron3DGraphNode node4)
        {
            // решение системы лин. уравнений (3x3), используемое для проверки связи 1-2 на локальную выпуклость (см. алгоритм)
        ...
        private Matrix SolveCone123EquationSystem(Polyhedron3DGraphNode node1, Polyhedron3DGraphNode node2, Polyhedron3DGraphNode node3)
        {
        ...
        private Matrix CalcLambda123(Vector3D coneVector1, Vector3D coneVector2, Vector3D coneVector3, Vector3D coneVector4)
        {
            Matrix matrixA = new Matrix(3, 3);
        ...
        private Matrix CalcLambda123(Vector3D coneVector1, Vector3D coneVector2, Vector3D coneVector3, Vector3D coneVector4, out Matrix matrixError)
        {
            Matrix matrixA = new Matrix(3, 3);
            matrixA[1, 1] = coneVector1.XCoord;
            matrixA[1, 2] = coneVector2.XCoord;
...
#warning может нужна настоящая проверка ???
        ...
        private void SectorTriangulation(SuspiciousConnectionSet connSet, ICyclicList<Polyhedron3DGraphNode> sectorNodeList)
        {
            ...
            for (Int32 sectorNodeIndex = 0; sectorNodeIndex < sectorNodeList.Count; ++sectorNodeIndex)
            {
                ...
                for (Int32 dirtyVisibleNodeIndex = 0; dirtyVisibleNodeIndex < dirtyVisibleNodes.Count; ++dirtyVisibleNodeIndex)
                {
                ...
                for (Int32 visibleNodeIndex = 0; visibleNodeIndex < visibleNodes.Count; ++visibleNodeIndex)
                {
                    ...
                    currentNode.ConnectionList.Insert(currentNode.ConnectionList.IndexOf(beforeCurrentNode),
                                                      currentVisibleNode);
                    currentVisibleNode.ConnectionList.Insert(currentVisibleNode.ConnectionList.IndexOf(sectorNodeList.GetPrevItem(currentVisibleNode)),
                                                             currentNode);
                    // проверить правильно ли ???
                    ...
                    CyclicList<Polyhedron3DGraphNode> subSector = new CyclicList<Polyhedron3DGraphNode>();
...
#warning может более специализированное исключение
        ...
        /// <returns></returns>
        private List<Polyhedron3DGraphNode> GetVisibleNodes(ICyclicList<Polyhedron3DGraphNode> sectorNodeList, Polyhedron3DGraphNode initialNode)
        {
            List<Polyhedron3DGraphNode> visibleNodes = new List<Polyhedron3DGraphNode>();
        ...
        /// <returns></returns>
        private Boolean CheckNodeVisibility(ICyclicList<Polyhedron3DGraphNode> sectorNodeList, Polyhedron3DGraphNode initialNode, Polyhedron3DGraphNode checkedNode)
        {
            // для единообразия
            Polyhedron3DGraphNode node1 = initialNode;
            ...
            for (Polyhedron3DGraphNode nodei = node2; nodei != nodem; nodei = sectorNodeList.GetNextItem(nodei))
            {
                ...
                Vector3D crossingVector = Vector3D.VectorProduct(Vector3D.VectorProduct(node1.NodeNormal, nodek.NodeNormal),
                ...
                Double lambda3 = (scalarProducti - scalarProductip1 * scalarProductiip1) / (1 - scalarProductiip1 * scalarProductiip1);
                Double lambda4 = (scalarProductip1 - scalarProducti * scalarProductiip1) / (1 - scalarProductiip1 * scalarProductiip1);
        ...
        /// <param name="checkedNode"></param>
        /// <returns></returns>
        private Boolean CheckConnInSector(List<Polyhedron3DGraphNode> visibleNodes, Polyhedron3DGraphNode initialNode, Polyhedron3DGraphNode checkedNode)
        {
