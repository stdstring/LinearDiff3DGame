...
namespace LinearDiff3DGame.Geometry3D
{
    ...
    public class Polyhedron3DFromPointsFactory
    {
            ...
            for (Int32 sideIndex = 0; sideIndex < sideList.Count; ++sideIndex)
            {
                ...
                for (Int32 csVertexIndex = 0; csVertexIndex < currentSide.VertexList.Count; ++csVertexIndex)
                {
                    ...
                    for (Int32 vertexIndex = 0; vertexIndex < vertexList.Count; ++vertexIndex)
                    {
                        ...
                        if (Object.ReferenceEquals(leftEdgeVertex, currentVertex) || 
                            Object.ReferenceEquals(rightEdgeVertex, currentVertex))
                        {
                            continue;
                        }
                        ...
                        if (checkResult)
                        {
                            ...
                            Vector3D externalNormal = GetSideExternalNormal(vertexList, leftEdgeVertex, rightEdgeVertex, currentVertex);
                            ...
                            // добавление в список граней всех вершин, принадлежащих данной грани, данной грани
                            for (Int32 sideVertexIndex = 0; sideVertexIndex < orderVertexList.Count; ++sideVertexIndex)
                            {
                                orderVertexList[sideVertexIndex].SideList.Add(newSide);
                            }
        ...
        private Int32 GetSideCount4Edge(PolyhedronVertex3D edgeVertex1, PolyhedronVertex3D edgeVertex2)
        {
            ...
            {
                if (edgeVertex1.SideList[sideIndex].HasVertex(edgeVertex2)) ++sideCount4Edge;
            }
        ...
        private Boolean IsSideAlreadyAdded(List<PolyhedronSide3D> sideList, Vector3D externalNormal)
        {
            ...
            for (Int32 sideIndex = 0; sideIndex < sideList.Count; ++sideIndex)
            {
                Double angleBetweenVectors = Vector3D.AngleBetweenVectors(sideList[sideIndex].SideNormal, externalNormal);
                if (m_ApproxComparer.EQ(angleBetweenVectors, 0))
                {
                    checkResult = true;
                    break;
                }
        ...
        private PolyhedronSide3D GetFirstSide(List<PolyhedronVertex3D> vertexList)
        {
            PolyhedronSide3D firstSide = null;

            PolyhedronVertex3D vertex1 = null;
            PolyhedronVertex3D vertex2 = null;
            PolyhedronVertex3D vertex3 = null;

            List<PolyhedronVertex3D> sideVertexList = new List<PolyhedronVertex3D>();

            // Берем первую вершину из списка вершин в качестве первой вершины (первой) грани
            vertex1 = vertexList[0];
            ...
            for (Int32 vertex2Index = 1; vertex2Index < vertexList.Count; ++vertex2Index)
            {
                ...
                vertex2 = vertexList[vertex2Index];

                // Цикл по всем вершинам (кроме первой) из списка вершин
                for (Int32 vertex3Index = 1; vertex3Index < vertexList.Count; ++vertex3Index)
                {
                    // Если текущая вершина совпадает с первой или второй вершинами грани, то переход к следующей итерации
                    if (vertex2Index == vertex3Index) continue;
                    // Текущая вершина становится третей вершиной грани
                    vertex3 = vertexList[vertex3Index];
                    ...
                    if (checkResult)
                    {
                        ...
                        for (Int32 sideVertexIndex = 0; sideVertexIndex < orderVertexList.Count; ++sideVertexIndex)
                        {
                            orderVertexList[sideVertexIndex].SideList.Add(firstSide);
                        }
        ...
        private Boolean DoesVertexesFormSide(List<PolyhedronVertex3D> vertexList, List<PolyhedronVertex3D> sideVertexList)
        {
            ...
            for (Int32 vertexIndex = 0; vertexIndex < vertexList.Count; ++vertexIndex)
            {
                ...
                if (Object.ReferenceEquals(currentVertex, sideVertexList[0]) ||
                    Object.ReferenceEquals(currentVertex, sideVertexList[1]) ||
                    Object.ReferenceEquals(currentVertex, sideVertexList[2]))
                {
                    continue;
                }
        ...
        private Vector3D GetSideExternalNormal(List<PolyhedronVertex3D> vertexList, PolyhedronVertex3D vertex1, PolyhedronVertex3D vertex2, PolyhedronVertex3D vertex3)
        {
            Vector3D externalNormal = Vector3D.ZeroVector3D;

            if (Object.ReferenceEquals(vertex1, vertex2) ||
                Object.ReferenceEquals(vertex1, vertex3) ||
                Object.ReferenceEquals(vertex2, vertex3))
            {
#warning может более специализированное исключение
            ...
            for (Int32 vertexIndex = 0; (m_ApproxComparer.EQ(scalarProduct, 0) && vertexIndex < vertexList.Count); ++vertexIndex)
            {
                PolyhedronVertex3D currentVertex = vertexList[vertexIndex];
...
#warning может более специализированное исключение
        ...
        private List<PolyhedronVertex3D> OrderSideVertexList(List<PolyhedronVertex3D> sideVertexList, Vector3D externalNormal)
        {
            ...
            while (disorderVertexList.Count > 0)
            {
                ...
                for (Int32 disorderedVertexIndex = 1; disorderedVertexIndex < disorderVertexList.Count; ++disorderedVertexIndex)
                {
