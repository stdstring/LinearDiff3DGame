...
{
    [TestFixture]
    public class Vector3DExtensions_Test
    {
        [Test]
        public void ToPoint3D()
        {
            Assert.AreEqual(new Point3D(1.0, 2.1, 1.2), new Vector3D(1.0, 2.1, 1.2).ToPoint3D());
        }
        ...
        public void ToMatrixColumn()
        {
            ...
            Assert.IsTrue(comparer.Equals(expectedColumn, new Vector3D(1.0, 2.1, 1.2).ToMatrixColumn()));
        }
        ...
        public void ToVector3D()
        {
            Assert.Throws<IncorrectMatrixSizeException>(
                () => new MatrixFactory().CreateFromRawData(2, 2, 1.0, 1.0, 0.0, 0.0).ToVector3D());
            Matrix column = new MatrixFactory().CreateFromRawData(3, 1, 1.0, 2.1, 1.2);
            Assert.AreEqual(new Vector3D(1.0, 2.1, 1.2), column.ToVector3D());
        }
    }
}